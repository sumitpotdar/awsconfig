...
Environment Variables:
    ...
    RESOURCE_TYPES           - (optional) comma-separated resource types to include, e.g.
                               "AWS::EC2::Volume,AWS::SNS::Topic,AWS::ApiGateway::RestApi,AWS::ApiGatewayV2::Api"
                               Leave blank to report ALL non-compliant resource types.
"""
...
# Comma-separated resource types to filter on — case-insensitive match.
# e.g. "AWS::EC2::Volume,AWS::SNS::Topic,AWS::ApiGateway::RestApi,AWS::ApiGatewayV2::Api"
# Leave blank to report ALL non-compliant resource types.
_rt_env             = os.environ.get("RESOURCE_TYPES", "")
RESOURCE_TYPE_FILTER = {t.strip().lower() for t in _rt_env.split(",") if t.strip()}
    ...
    for rule_info in all_rules:
        ...
        while True:
            ...
            for ev in resp.get("AggregateEvaluationResults", []):
                ...
                # Apply resource-type filter (case-insensitive)
                if RESOURCE_TYPE_FILTER and resource_type.lower() not in RESOURCE_TYPE_FILTER:
                    continue
                all_results.append({
...
def build_html_report(all_rows: list) -> str:
    ...
    html = [
        ...
        (f"<p>Resource type filter: <code style='color:#90cdf4'>" + ", ".join(sorted(RESOURCE_TYPE_FILTER)) + "</code></p>" if RESOURCE_TYPE_FILTER else ""),
        "</div>",
    ]
...
def lambda_handler(event, context):
    ...
    if RESOURCE_TYPE_FILTER:
        logger.info("Resource type filter active: %s", sorted(RESOURCE_TYPE_FILTER))
    else:
        logger.info("No resource type filter — reporting ALL non-compliant resource types")
