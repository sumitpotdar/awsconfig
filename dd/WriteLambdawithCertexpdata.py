...
def process_instance(rds, db, account_id, account_name, region, scan_timestamp):
    ...
    # Get SSL configuration with certificate expiry date
...
def get_ssl_configuration(rds, db, region):
    ...
    try:
        ...
        if ca_cert:
            ...
            # Get certificate details including expiry date
            try:
                ...
                if cert_response.get('Certificates'):
                    ...
                    # Get the ValidTill date (expiry date)
        ...
        # Check if SSL is enforced via parameter group
