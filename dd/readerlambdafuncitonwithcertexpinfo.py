...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Debug: Log encryption data for first instance
        if DEBUG_MODE and result.get('instances') and len(result['instances']) > 0:
            first_instance = result['instances'][0]
            print(f"=== Sample Instance Data ===")
            print(f"Instance ID: {first_instance.get('id')}")
            print(f"Encryption object: {json.dumps(first_instance.get('encryption', {}), cls=DecimalEncoder)}")
            if first_instance.get('encryption'):
                print(f"  - certificateExpiryDate: {first_instance['encryption'].get('certificateExpiryDate')}")
                print(f"  - certificateDaysToExpiry: {first_instance['encryption'].get('certificateDaysToExpiry')}")
                print(f"  - inTransit: {first_instance['encryption'].get('inTransit')}")
                print(f"  - certificateAuthority: {first_instance['encryption'].get('certificateAuthority')}")
...
def get_latest_scan():
    ...
    try:
        ...
        if not instances:
            print(f"No data found for {today}, trying {yesterday}")
            instances = query_by_date(yesterday)
        
        # If still no data, try last 7 days
        if not instances:
            print("No data found for today or yesterday, searching last 7 days")
            for days_back in range(2, 8):
                date = (datetime.now() - timedelta(days=days_back)).strftime('%Y-%m-%d')
                print(f"Trying {date}...")
                instances = query_by_date(date)
                if instances:
                    print(f"Found data from {date}")
                    break
...
def query_instances(filters):
    ...
    print(f"Filters received: date={date}, account={account_id}, tag={tag_key}={tag_value}, region={region}")
    ...
    if date:
        ...
        # Default to latest
        print("No date/account filter provided - getting latest scan")
        latest_result = get_latest_scan()
        instances = latest_result.get('instances', [])
    
    print(f"Retrieved {len(instances)} instances before filtering")
    
    # Apply additional filters
    if tag_key and tag_value:
        print(f"Applying tag filter: {tag_key}={tag_value}")
        instances = filter_by_tag(instances, tag_key, tag_value)
        print(f"After tag filter: {len(instances)} instances")
    
    if account_id and date:
        instances = [i for i in instances if i.get('account_id') == account_id]
        print(f"After account filter: {len(instances)} instances")
    
    if region:
        instances = [i for i in instances if i.get('region') == region]
        print(f"After region filter: {len(instances)} instances")
    
    print(f"Final result: {len(instances)} instances")
...
def query_by_date(scan_date):
    ...
    try:
        ...
        print(f"Retrieved {len(items)} total items before deduplication")
        ...
        for item in items:
            ...
            if instance_id not in instances_map:
                # Debug: Log which timestamp we're taking
                if DEBUG_MODE and len(instances_map) < 3:  # Log first 3 instances
                    print(f"  Taking instance '{instance_id}' from timestamp: {item.get('scan_timestamp')}")
                    has_cert = item.get('encryption', {}).get('certificateExpiryDate') is not None
                    print(f"    Has certificate data: {has_cert}")
                
                instances_map[instance_id] = clean_instance_data(item)
        
        instances = list(instances_map.values())
        print(f"Found {len(instances)} unique instances for date {scan_date}")
...
def query_by_account(account_id):
    ...
    try:
        ...
        instances = list(instances_map.values())
        print(f"Found {len(instances)} unique instances for account {account_id}")
...
def filter_by_tag(instances, tag_key, tag_value):
    ...
    print(f"Total instances to filter: {len(instances)}")
    ...
    for instance in instances:
        ...
        if len(filtered) == 0 and DEBUG_MODE:
            print(f"Sample instance tags: {tags}")
            print(f"Looking for: {tag_key}={tag_value}")
            print(f"Tag key exists: {tag_key in tags}")
            if tag_key in tags:
                print(f"Tag value match: {tags[tag_key]} == {tag_value} ? {tags[tag_key] == tag_value}")
    ...
    # If no matches, show available tags for debugging
    if len(filtered) == 0 and len(instances) > 0:
        print("No matches found. Sample tags from first instance:")
        sample_tags = instances[0].get('tags', {})
        for k, v in sample_tags.items():
            print(f"  {k} = {v}")
...
def clean_instance_data(item):
    ...
    if DEBUG_MODE:
        ...
        if item.get('encryption'):
            print(f"BEFORE clean_instance_data for {instance_id}:")
            print(f"  encryption keys: {list(item['encryption'].keys())}")
            print(f"  certificateExpiryDate: {item['encryption'].get('certificateExpiryDate')}")
            print(f"  certificateDaysToExpiry: {item['encryption'].get('certificateDaysToExpiry')}")
    ...
    # Keep all application data including:
    # - encryption (with certificateExpiryDate, certificateDaysToExpiry)
    # - authentication
    # - secrets
    # - backups
    # - network
    ...
    if DEBUG_MODE:
        instance_id = item.get('id', 'unknown')
        if item.get('encryption'):
            print(f"AFTER clean_instance_data for {instance_id}:")
            print(f"  encryption keys: {list(item['encryption'].keys())}")
            print(f"  certificateExpiryDate: {item['encryption'].get('certificateExpiryDate')}")
            print(f"  certificateDaysToExpiry: {item['encryption'].get('certificateDaysToExpiry')}")
