...
# ─────────────────────────────────────────────────────────────────────────────
# Caches — ALL keyed by (account_id, ...) to prevent cross-account pollution.
#
# BUG 1 FIX: _secrets_cache key is now (account_id, region) not just region.
#   Old: _secrets_cache['us-east-1'] — Account A's secrets returned for Account B.
#   New: _secrets_cache[('accountA', 'us-east-1')] vs
#        _secrets_cache[('accountB', 'us-east-1')] — fully isolated.
#
# BUG 3 FIX: _param_group_cache key is now (account_id, pg_name) not pg_name.
#   Same default parameter group name (e.g. 'default.mysql8.0') exists in every
#   account — old key returned Account A's SSL result for Account B.
# ─────────────────────────────────────────────────────────────────────────────
_secrets_cache: dict     = {}   # (account_id, region)  -> list[secret_dict]
_param_group_cache: dict = {}   # (account_id, pg_name) -> bool (ssl_enforced)
...
def _load_secrets_for_region(region: str,
    ...
    BUG 1 FIX: Cache key is (account_id, region) — not region alone.
               Previously all accounts shared the same region bucket so
               cross-account instances always got Account A's secrets.

    BUG 2 FIX: IAM/permission failures are now printed as clear [WARN] lines
               instead of being silently swallowed into an empty list.
               Operators can now see in CloudWatch Logs exactly which permission
               is missing on the cross-account role.
    """
    ...
    try:
        ...
        try:
            ...
            # BUG 2 FIX: surface IAM permission failures clearly
            code = e.response['Error']['Code']
            print(f"  [WARN] {account_id}/{region}: secretsmanager:ListSecrets FAILED"
                  f" ({code}). Ensure {ROLE_NAME} has secretsmanager:ListSecrets"
                  f" and secretsmanager:DescribeSecret. Error: {e}")
        ...
        def _describe(stub):
            ...
            try:
                ...
                # BUG 2 FIX: surface describe_secret failures
                code = e.response['Error']['Code']
                print(f"  [WARN] describe_secret FAILED for {name} ({code}): {e}")
                return stub
            except Exception as e:
                print(f"  [WARN] describe_secret error for {name}: {e}")
                return stub
        ...
        # BUG 2 FIX: catch-all also surfaces the error clearly
        print(f"  [WARN] {account_id}/{region}: unexpected error loading secrets — {e}")
...
def match_secret_to_instance(secret_name: str,
    ...
    if m:
        rds_app_name = m.group('rds_app_name')
...
def _scan_region(region, account_id, account_name, credentials, scan_timestamp):
    ...
    try:
        ...
        if not db_stubs:
            print(f"  [{region}] No RDS instances found")
            return instances

        print(f"  [{region}] {len(db_stubs)} instances found"
              f" — pre-loading secrets cache and fetching tags in parallel")

        # ── Pre-load secrets cache for this account+region BEFORE instance loop ──
        # All subsequent check_secrets_manager() calls will be cache-hits (zero API calls).
        ...
        def _fetch_tags(db):
            try:
                ...
                print(f"  [WARN] list_tags_for_resource failed for"
                      f" {db['DBInstanceIdentifier']}: {e}")
                return db['DBInstanceIdentifier'], {}
        ...
        if e.response['Error']['Code'] == 'UnauthorizedOperation':
            print(f"  [WARN] Not authorized for region {region} in account {account_id}")
        else:
...
def scan_account(account_id, account_name, credentials, scan_timestamp):
    ...
    NOTE: _secrets_cache.clear() has been intentionally removed.
    With (account_id, region) cache keys each account's secrets are fully
    isolated — the old .clear() was a broken workaround that also wiped
    valid cache entries on Lambda retries.
    """
...
def process_instance(rds, db, account_id, account_name, region,
    ...
    BUG 4 FIX: Previously tags['app_id'] was overwritten with account_id
               (e.g. '123456789012') for instances missing the app_id AWS tag,
               and these MUTATED tags were passed straight into check_secrets_manager.
               This caused inst_app_id = '123456789012' which can never equal any
               secret's app_id tag value (e.g. 'pocapp') — so ALL secrets matching
               failed silently for those instances.

               Fix: keep original_tags (the real AWS tags, never mutated) for
               secrets matching.  Use a separate storage_tags copy — which may have
               the defaulted app_id — only for DynamoDB storage and display.
    """
    ...
    if prefetched_tags is not None:
        raw_tags = prefetched_tags
    else:
        try:
            tags_response = rds.list_tags_for_resource(ResourceName=db_arn)
            raw_tags = {tag['Key']: tag['Value'] for tag in tags_response['TagList']}
        except Exception:
            raw_tags = {}

    # BUG 4 FIX: two separate tag dicts — never mutate original_tags
    original_tags = dict(raw_tags)   # real AWS tags — used for secrets matching
    storage_tags  = dict(raw_tags)   # copy for DynamoDB — may have defaulted app_id

    has_app_id = any(k in storage_tags for k in ('app_id', 'APP_ID', 'AppId',
                                                   'application_id', 'ApplicationId'))
    app_id_defaulted = not has_app_id
    if app_id_defaulted:
        # Write the default ONLY into storage_tags, NOT original_tags
        storage_tags['app_id'] = account_id
        print(f"  [{instance_id}] Missing app_id tag — defaulting to account_id"
              f" for DynamoDB storage only (secrets matching uses original AWS tags)")

    # BUG 3 FIX: account_id passed to get_ssl_configuration for scoped pg cache
    ssl_info = get_ssl_configuration(rds, db, region, account_id)

    # BUG 4 FIX: pass original_tags so secrets matching is against real AWS tags
    secrets_info = check_secrets_manager(
        instance_id, original_tags, region, credentials, account_id
    )
    ...
    return {
        ...
        'arn':              db_arn,
        'tags':             storage_tags,    # includes defaulted app_id for UI
...
def get_ssl_configuration(rds, db, region, account_id: str = 'local'):
    """
    Get SSL/TLS configuration including certificate expiry date.

    BUG 3 FIX: _param_group_cache key is now (account_id, pg_name).
               Old key was pg_name alone — every AWS account has a parameter
               group called 'default.mysql8.0' (or similar), so Account A's
               SSL enforcement result was being returned for Account B.
    """
    ssl_info = {
    ...
    try:
            ...
            try:
                ...
                if cert_response.get('Certificates'):
                    ...
                    if certificate.get('ValidTill'):
                        ...
                        print(f"  Certificate {ca_cert} expires"
                              f" {valid_till:%Y-%m-%d} ({days}d)")
                        if days < 30:
        ...
        if db.get('DBParameterGroups'):
            ...
            cache_key = (account_id, pg_name)   # BUG 3 FIX: scoped per account

            if cache_key in _param_group_cache:
                ssl_info['enforced'] = _param_group_cache[cache_key]
            else:
                enforced = False
                try:
                    ...
                    for page in paginator.paginate(DBParameterGroupName=pg_name):
                        for param in page.get('Parameters', []):
                            ...
                            if (name in ('rds.force_ssl', 'require_secure_transport')
                                    and value == '1'):
                                enforced = True
                                break
                        if enforced:
                    ...
                    print(f"  Error checking SSL enforcement for {pg_name}: {e}")
                _param_group_cache[cache_key] = enforced
...
def check_secrets_manager(instance_id: str,
    ...
    BUG 1 FIX: account_id is threaded through to _load_secrets_for_region so
               the correct per-account cache bucket is used.

    BUG 4 FIX: Caller now passes original_tags (not storage_tags that may have
               app_id defaulted to account_id '123456789012'), so matching uses
               the real AWS tag values.
    ...
    try:
        # BUG 1 FIX: account-scoped cache lookup
        all_region_secrets = _load_secrets_for_region(region, credentials, account_id)

        # BUG 2 FIX: if cache is empty, surface it clearly so operators know
        if not all_region_secrets:
            print(f"  [{instance_id}] Secrets cache empty for {account_id}/{region}"
                  f" — if unexpected, check secretsmanager:ListSecrets permission"
                  f" on role {ROLE_NAME}")
        ...
        print(f"  Error checking Secrets Manager for {instance_id}: {e}")
...
def _strip_none(obj):
    ...
    or raises a serialization error.
    """
...
def store_instances(instances, scan_date, scan_timestamp):
    """Store instances in DynamoDB with deduplication."""
    unique_items = {}
