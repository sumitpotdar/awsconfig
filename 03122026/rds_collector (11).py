...
def scan_account(account_id, account_name, credentials, scan_timestamp):
    ...
    # Fixed scan regions — only us-east-1 and us-west-2
...
def check_secrets_manager(instance_id: str,
    ...
    The cache is populated once per region so this function makes zero
...
def store_instances(instances, scan_date, scan_timestamp):
    ...
    with table.batch_writer() as batch:
        for instance in instances:
            # 1. JSON round-trip converts floats → Decimal (required by DynamoDB)
            # 2. _strip_none removes any None values — DynamoDB rejects None and
            #    silently drops nested attributes that contain it (e.g. all_secrets)
            item = _strip_none(
