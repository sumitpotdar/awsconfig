...
Connection model
----------------
  All connection details are read from a single Secrets Manager secret.
  DR_SECRET_ARN (Lambda env var) points to the ARN of that secret.

  The secret must be a JSON object containing:
    {
      "host":     "mydr-cluster.cluster-xyz.us-east-1.rds.amazonaws.com",
      "username": "druser",
      "password": "s3cr3t!",
      "dbname":   "drdb",
      "port":     5432
  ...
  NOTE: "port" is optional — defaults to 5432 (PostgreSQL) or 3306 (MySQL).
...
DR_SECRET_ARN    = os.environ.get('DR_SECRET_ARN',    '')   # SM ARN -> host/username/password/dbname/port
...
def fetch_dr_records() -> list:
    """
    Connects to the external DR database using connection details fetched
    entirely from Secrets Manager (DR_SECRET_ARN).
    ...
    """
    host, port, dbname, username, password = _get_connection_params()
    conn = _open_db_connection(host, port, dbname, username, password)

    # Identifier quoting handles column names that contain spaces:
...
def _get_connection_params() -> tuple:
    """
    Fetches all DB connection parameters from Secrets Manager and returns:
        (host, port, dbname, username, password)

    All fields come from the secret JSON stored at DR_SECRET_ARN:
      host      <- secret JSON field 'host'     (required — DB endpoint/hostname)
      port      <- secret JSON field 'port'     (optional — defaults 5432/3306)
    ...
    if not DR_SECRET_ARN:
        raise ValueError(
            ...
            "Set it to the Secrets Manager ARN containing host/username/password/dbname."
        )

    # Retrieve all connection details from Secrets Manager
    logger.info(f"Fetching DB connection details from Secrets Manager: {DR_SECRET_ARN}")
    try:
    ...
    # Validate all required fields are present in the secret
    missing = [k for k in ('host', 'username', 'password', 'dbname') if k not in secret]
    if missing:
        raise ValueError(
            f"Secret {DR_SECRET_ARN} is missing required field(s): {missing}. "
            "Secret must contain: host, username, password, dbname -- and optionally port."
        )
    ...
    logger.info(f"Connection params resolved -- "
                f"host={host} port={port} dbname={dbname} user={username}")

    return host, port, dbname, username, password


def _open_db_connection(host: str, port: int, dbname: str,
    ...
    if DR_DB_ENGINE == 'postgresql':
        import psycopg2
        logger.info(f"Opening PostgreSQL connection -> {host}:{port}/{dbname}")
        ...
        import pymysql
        logger.info(f"Opening MySQL connection -> {host}:{port}/{dbname}")
