...
def lambda_handler(event, context):
    ...
    # FIX: Always use LOCAL account to get region list.
    # This avoids needing ec2:DescribeRegions on the cross-account role
    # and prevents "Not authorized" errors on opt-in regions.
    regions = get_regions()
    ...
    try:
        ...
        for account in accounts:
            ...
            try:
                ...
                instances = scan_account(account_id, account_name, credentials, scan_timestamp, regions)
...
def get_regions():
    """
    Fetch available AWS regions using the LOCAL account credentials.
    This avoids requiring ec2:DescribeRegions on cross-account roles.
    """
    try:
        local_ec2 = boto3.client('ec2', region_name='us-east-1')
        regions = [
            r['RegionName'] for r in local_ec2.describe_regions(
                Filters=[{'Name': 'opt-in-status', 'Values': ['opt-in-not-required', 'opted-in']}]
            )['Regions']
        ]
        print(f"Fetched {len(regions)} regions from local account")
        return regions
    except Exception as e:
        print(f"Could not fetch regions dynamically, using defaults: {e}")
...
def get_boto3_client(service, region, credentials=None):
    """
    Helper to create a boto3 client with or without cross-account credentials.
    Centralises credential handling to avoid repetition across functions.
    """
    if credentials:
        return boto3.client(
            service,
...
def scan_account(account_id, account_name, credentials, scan_timestamp, regions):
    """Scan all EC2 instances in all regions for a given account"""
    instances = []

    for region in regions:
        try:
            # FIX: Use helper so cross-account credentials are always applied
            ec2_client = get_boto3_client('ec2', region, credentials)
            ...
            for page in paginator.paginate():
                for reservation in page['Reservations']:
                    for instance in reservation['Instances']:
                        try:
                            # FIX: Pass credentials through to process_instance
                            instance_data = process_instance(
                                ec2_client, instance, account_id, account_name,
                                region, scan_timestamp, credentials
                            )
            ...
            if region_instance_count > 0:
                print(f"  Region {region}: found {region_instance_count} instances")

        except ClientError as e:
            error_code = e.response['Error']['Code']
            if error_code in ('UnauthorizedOperation', 'AuthFailure', 'AccessDenied'):
                print(f"  Not authorized for region {region} in account {account_id} - skipping")
            elif error_code == 'OptInRequired':
                print(f"  Region {region} not opted-in for account {account_id} - skipping")
            else:
                print(f"  Error in region {region} for account {account_id}: {str(e)}")
            continue
        except Exception as e:
            print(f"  Unexpected error in region {region} for account {account_id}: {str(e)}")
            continue

    return instances


def process_instance(ec2_client, instance, account_id, account_name, region, scan_timestamp, credentials=None):
    ...
    # FIX: Pass credentials so SSM check runs in the correct account
    ssm_info = check_ssm_status(instance_id, region, account_id, credentials)
...
def check_ssm_status(instance_id, region, account_id, credentials=None):
    """
    Check if instance is managed by Systems Manager.
    FIX: Now accepts credentials so it queries the correct (cross-)account.
    """
    ssm_info = {
        'managed': False,
        'ping_status': None,
    ...
    try:
        # FIX: Use helper to build SSM client with the correct credentials
        ssm = get_boto3_client('ssm', region, credentials)
        ...
        if error_code in ('AccessDeniedException', 'UnauthorizedOperation'):
            print(f"  SSM access denied for instance {instance_id} in account {account_id} region {region}")
        # Other errors (e.g. SSM not available) are silently ignored
    except Exception:
        pass
